...
if(FORCE_HTTPS && location.protocol==='http:')
	...
	navigator.serviceWorker.register('/rb/_cache_service_worker.js', {scope: '/rb/'});
...
try{
	webWorkerApply=new Worker('/rb/js/worker_apply.js');
	...
	webWorkerCreate=new Worker('/rb/js/worker_create.js');
	...
	webWorkerCrc=new Worker('/rb/js/worker_crc.js');
...
addEvent(window,'load',function(){
	...
	if(CAN_USE_WEB_WORKERS){
		...
		zip.workerScriptsPath='/rb/js/zip.js/';
	}else{
		zip.useWebWorkers=false;

		var script=document.createElement('script');
		script.src='/rb/js/zip.js/inflate.js';
